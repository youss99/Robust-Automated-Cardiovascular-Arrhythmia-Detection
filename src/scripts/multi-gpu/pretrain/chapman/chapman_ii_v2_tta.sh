#!/bin/bash
# -----------------------------------------------------------------------------
# Chapman pretrain — two-lead (II + V2), TTA-ready
#
# Reproduces params.txt from:
#   results/pre-train/saved_models/chapman/patch_ecg_chapman_ii_v2_tta/
#
# Submit:  sbatch src/scripts/multi-gpu/pretrain/chapman/chapman_ii_v2_tta.sh
# -----------------------------------------------------------------------------
...
# Stage chapman to fast local NVMe ($SLURM_TMPDIR) for training
...
torchrun --standalone --nproc_per_node=1 -m src.patchECG_pretrain \
    ...
    --model_name=patch_ecg_chapman_ii_v2_tta \
    --min_class_size=50 \
    --context_points=5000 \
    --target_points=100 \
    --scaler=standard \
