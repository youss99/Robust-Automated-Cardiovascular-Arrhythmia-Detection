#!/bin/bash
# -----------------------------------------------------------------------------
# Chapman pretrain — lead II only
#
# Reproduces params.txt from:
#   results/pre-train/saved_models/chapman/patch_ecg_chapman_ii_only/
#
# Submit:  sbatch src/scripts/multi-gpu/pretrain/chapman/chapman_ii_only.sh
# -----------------------------------------------------------------------------
#SBATCH --account=def-majidk
#SBATCH --job-name=chapman_ii_only
#SBATCH --nodes=1
...
#SBATCH --output=logs/chapman_ii_only-%j.out
...
torchrun --standalone --nproc_per_node=1 -m src.patchECG_pretrain \
    ...
    --custom_lead_selection="II" \
    --model_name=patch_ecg_chapman_ii_only \
    --min_class_size=50 \
    --context_points=5000 \
    --target_points=100 \
    --scaler=standard \
