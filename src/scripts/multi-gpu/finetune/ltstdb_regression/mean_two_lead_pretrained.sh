#!/bin/bash
# -----------------------------------------------------------------------------
# LTSTDB regression fine-tune — MEAN target, two-lead (II+V2), PRETRAINED
#   from chapman two-lead (patch_ecg_chapman_ii_v2_tta_best.pt)
#
# Same params as median_two_lead_pretrained.sh; differs only in dataset path
# and model_name suffix.
#
# Submit:  sbatch src/scripts/multi-gpu/finetune/ltstdb_regression/mean_two_lead_pretrained.sh
# -----------------------------------------------------------------------------
#SBATCH --account=def-majidk
#SBATCH --job-name=ltst_mean_pre
#SBATCH --nodes=1
...
#SBATCH --output=logs/ltstdb_mean_two_lead_pretrained-%j.out
...
DATA_SRC="$REPO_ROOT/custom_datasets/ltstdb_regression_mean_100hz_full16_12_2_2_omit_train_nocompress"
...
echo "Staging mean dataset to $SLURM_TMPDIR ..."
...
torchrun --standalone --nproc_per_node=1 -m src.patchECG_finetune \
    ...
    --patch_len=50 \
    --stride=50 \
    --model_name=ltstdb_mean_full16_12_2_2_omit_train_two_lead_regression_pretrained_chapman_ii_v2_trial01 \
    --metric_by_class \
    --no-mentor_mix \
    --no-bootstrapping \
    --iterations=5000 \
