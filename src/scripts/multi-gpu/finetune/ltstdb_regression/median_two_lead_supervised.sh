#!/bin/bash
# -----------------------------------------------------------------------------
# LTSTDB regression fine-tune — MEDIAN target, two-lead (II+V2), SUPERVISED
#
# Mirrors params.txt from local run:
#   results/fine-tune/saved_models/patient-segment-regression/
#     ltstdb_median_full16_12_2_2_omit_train_two_lead_regression_supervised_trial01/
#
# Submit:  sbatch src/scripts/multi-gpu/finetune/ltstdb_regression/median_two_lead_supervised.sh
# -----------------------------------------------------------------------------
#SBATCH --account=def-majidk
#SBATCH --job-name=ltst_med_sup
#SBATCH --nodes=1
...
#SBATCH --output=logs/ltstdb_median_two_lead_supervised-%j.out
...
torchrun --standalone --nproc_per_node=1 -m src.patchECG_finetune \
    ...
    --patch_len=50 \
    --stride=50 \
    --model_name=ltstdb_median_full16_12_2_2_omit_train_two_lead_regression_supervised_trial01 \
    --metric_by_class \
    --no-mentor_mix \
    --no-bootstrapping \
    --iterations=5000 \
