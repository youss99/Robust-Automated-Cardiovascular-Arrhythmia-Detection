#!/bin/bash
# PTB-XL-backbone -> Unified fine-tune | thesis Small, two-lead II+V2 | Config 2: Focal + TTA
#SBATCH --account=def-majidk
#SBATCH --job-name=p2u_c2_ft
#SBATCH --nodes=1
...
#SBATCH --output=logs/patch_ecg_ptbxl2unified_ii_v2_c2_focal_tta-%j.out
set -uo pipefail
...
PRETRAINED_CKPT="$REPO_ROOT/results/pre-train/saved_models/ptb-xl/patch_ecg_ptbxl_ii_v2/patch_ecg_ptbxl_ii_v2_best.pt"
...
torchrun --standalone --nproc_per_node=1 -m src.patchECG_finetune \
    ...
    --model_name=patch_ecg_ptbxl2unified_ii_v2_c2_focal_tta
